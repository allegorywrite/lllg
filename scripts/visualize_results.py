...
def add_mean_line_to_scatter(df, grouping_column, x_col, y_col, line_property, is_collab_plot):
    """散布図に平均値を線で結ぶ機能を追加する"""
    import re
    from collections import defaultdict
    ...
    if is_collab_plot:
        ...
        for line_value, group_data in line_groups.items():
            if len(group_data) < 2:
            ...
            plt.plot(x_values, y_values, linestyle='-', color=color, alpha=0.8, linewidth=4, zorder=10, 
                    label=f'{line_property}={label_value}')
                ...
                for prop, value in other_properties.items():
                    if prop != line_property:
                        if isinstance(value, float) and value.is_integer():
                            ...
                            if idx == 0:
                                label_parts.append(f'I={int(value)}')
                            else:
                ...
                for prop, value in other_properties.items():
                    if prop != line_property:
                        if isinstance(value, float) and value.is_integer():
                            # 最初のデータ点のみプロパティ名を含める
                            if idx == 0:
                                label_parts.append(f'{prop}={int(value)}')
                            else:
                                label_parts.append(str(int(value)))
                        elif isinstance(value, bool) or str(value).lower() in ['true', 'false']:
                            # ブール値の場合
...
def plot_scatter(df, output_dir, vary_property, baseline_data=None, plot_settings=None):
    ...
    if vary_property: # vary_property は単独パラメータ名、またはコラボキー名
        ...
        if custom_title:
            plt.title(custom_title, fontweight='bold', fontsize=24, y=1.05)
        else:
            # デフォルトのタイトル生成
            if y_col == 'soc_normalized':
                plt.title(f"Normalized SOC vs {x_label} (by {vary_property})", 
                         fontweight='bold', fontsize=24, y=1.05)
            elif y_col == 'benchmark_normalized':
                plt.title(f"Benchmark Normalized SOC vs {x_label} (by {vary_property})", 
                         fontweight='bold', fontsize=24, y=1.05)
            else:
                plt.title(f"Total Cost (SoC) vs {x_label} (by {vary_property})", 
                         fontweight='bold', fontsize=24, y=1.05)
        
        # y軸ラベルを設定（カスタムラベルまたはデフォルトラベル）
        if custom_y_label:
            plt.ylabel(y_label, fontweight='bold', fontsize=22)
        else:
            # デフォルトのy軸ラベル生成
            if y_col == 'soc_normalized':
                plt.ylabel("Normalized SOC (SOC/SOC_LB)", fontweight='bold', fontsize=22)
            elif y_col == 'benchmark_normalized':
                plt.ylabel("Normalized SOC (SOC/Benchmark_SOC)", fontweight='bold', fontsize=22)
            else:
                plt.ylabel("Total Cost (sum_of_costs)", fontweight='bold', fontsize=22)
        ...
        if plot_settings:
            ...
            if line_property:
                add_mean_line_to_scatter(df, grouping_column, x_col, y_col, line_property, is_collab_plot)
